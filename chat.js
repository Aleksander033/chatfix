(function () {
  ...
  function nicknameEl() {
    return qs('#nick') || qs('#nickname') || qs('input[name="nick"]') || qs('input[name="nickname"]') || qs('input[placeholder*="nick" i]') || qs('input[placeholder*="name" i]');
  }
  ...
  function selectedServerUrl() {
    ...
    return clean(select?.value || localStorage.getItem('ZYNX:server') || 'eu.senpa.io:2001');
  }

  function normalizeServerUrl(url) {
    let value = clean(url, 'eu.senpa.io:2001')
      .replace(/^wss?:\/\//i, '')
      .replace(/^https?:\/\//i, '')
      .replace(/\?.*$/, '')
      .replace(/\/+$/, '');
    if (!value) value = 'eu.senpa.io:2001';
    return `wss://${value}`;
  }
  ...
  function getPrimaryClient() {
    ...
    const client = clients.find(c => c && c.clientID) || clients[0] || null;
    if (!client) {
      if (!window.fakeClientID) window.fakeClientID = Math.floor(Math.random() * 100000) + 1;
      return { clientID: window.fakeClientID, multiboxID: window.fakeClientID };
    }
    return client;
  }
  ...
  const chat = {
    ...
    sendPlayerUpdate(force = false) {
      ...
      const clientId = Number(client?.clientID || this.ClientId || 0);
